...
def main(args):
    ...
    valid_path = '/mnt/server4_hard2/jhjang/dh_pascal/OmniPose/data/mpii/annot/valid.json'
    with open(valid_path, 'r') as file:
        data = json.load(file)
        for i in range(len(data)):
            val_list.append(data[i]['image'])

    pdb.set_trace()

    for idx in range(len(val_list)):
        print(idx,"/",len(val_list))
        ...
        plot_COCO_image(4*preds, img_path, './samples/mpii_2/'+images[idx], colorstyle.link_pairs, colorstyle.ring_color, colorstyle.color_ids, save=True)
        # pdb.set_trace()
